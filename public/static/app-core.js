(() => {
  ...
  async function renderStudent(view) {
    ...
    if (view === 'dashboard') {
      ...
      ${resume?.ai_parsed_data ? `<div class="ai-box"><div class="ai-box-head"><span>PARSED RÉSUMÉ DATA</span><span>${(resume.ai_parsed_data.skills || []).length} skills detected</span></div>${tags(resume.ai_parsed_data.skills || [])}<p>${esc(resume.ai_parsed_data.summary || '')}</p></div>` : ''}</section>`;
