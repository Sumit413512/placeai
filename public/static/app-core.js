(() => {
  ...
  async function renderStudent(view) {
    ...
    if (view === 'dashboard') {
      ...
      const [d, apps, jobs, actionData] = await Promise.all([api('/students/dashboard'), api('/students/applications'), api('/students/jobs'), api('/intelligence/student/actions')]);
      const recentApps = apps.slice(0,4);
      const completion = d.profile_completion || 0;
      const nextActions = actionData.actions || [];
      $('#app-content').innerHTML = `${pageHead('Your next best actions','PlaceAI prioritizes what needs your attention now, then shows the supporting placement context.')}
        ${nextActions.length ? `<section class="panel"><div class="panel-head"><div><h2>Do these next</h2><p>Prioritized from deadlines, eligibility, preparation evidence and profile blockers.</p></div><button data-view="readiness">View readiness evidence</button></div><div class="activity-list">${nextActions.map((item,index)=>`<div class="activity-item"><span class="activity-icon">${String(index+1).padStart(2,'0')}</span><div><strong>${esc(item.title)}</strong><small>${esc(item.description)}${item.due_at ? ' · Due '+fmtDateTime(item.due_at) : ''}</small></div><div class="row-actions"><span class="status-badge status-${esc(item.priority)}">${esc(item.priority)}</span><button class="row-button primary" data-view="${esc(item.view)}">Open</button></div></div>`).join('')}</div></section>` : ''}
  ...
  async function renderInstitution(view) {
    ...
    if (view === 'dashboard') {
      ...
      await api('/intelligence/institution/actions/sync',{method:'POST'});
      const [d,rescue] = await Promise.all([api('/institutions/dashboard'),api('/intelligence/institution/actions')]);
      ...
      const rm=rescue.summary||{};
      $('#app-content').innerHTML = `${pageHead(`${d.organization.name} placement office`,'Your command centre starts with what needs action now, then the supporting placement metrics.','Add student','open-student-form')}
      <div class="metric-grid"><article class="metric-card"><small>Open rescue actions</small><strong>${rm.open_actions||0}</strong><span class="${rm.high_priority?'':'metric-good'}">${rm.high_priority||0} high priority</span></article><article class="metric-card"><small>Due within 24h</small><strong>${rm.due_within_24h||0}</strong><span>${rm.rescue_drives||0} drives need intervention</span></article><article class="metric-card"><small>Applications</small><strong>${d.total_applications}</strong><span>${d.offers} offers recorded</span></article><article class="metric-card"><small>Placement rate</small><strong>${d.placement_rate}%</strong><span class="${d.hires ? 'metric-good':''}">${d.hires} hires recorded</span></article></div>
      <div class="dashboard-grid"><section class="panel"><div class="panel-head"><div><h2>What needs attention today</h2><p>Objective placement exceptions detected from current workflow data.</p></div><button data-view="attention">Open Drive Rescue</button></div>${rescue.actions?.length?`<div class="activity-list">${rescue.actions.slice(0,6).map((x,index)=>`<div class="activity-item"><span class="activity-icon">${String(index+1).padStart(2,'0')}</span><div><strong>${esc(x.title)}</strong><small>${esc(x.student_name||'Student')} · ${esc(x.drive_title||x.description||'Placement action')}</small></div>${statusBadge(x.priority)}</div>`).join('')}</div>`:emptyState('OK','No rescue actions','No current placement exceptions require follow-up.')}</section><section class="panel"><div class="panel-head"><div><h2>Placement health</h2><p>Core institutional operating context</p></div><button data-view="analytics2">Open analytics</button></div><div class="progress-stack"><div class="progress-item"><span>Students verified</span><b>${d.verified_students}/${d.total_students}</b><div class="progress-bar"><i style="--p:${d.total_students?Math.max(2,Math.round(d.verified_students/d.total_students*100)):2}%"></i></div></div><div class="progress-item"><span>Open placement drives</span><b>${d.open_drives}</b></div><div class="progress-item"><span>Active campus jobs</span><b>${d.active_jobs}</b></div><div class="ai-box"><div class="ai-box-head"><span>INSTITUTION CODE</span><strong>${esc(d.organization.slug)}</strong></div><p>Student self-registration remains subject to placement-team verification.</p></div></div></section></div>`;
  ...
  document.addEventListener('click', async e => {
    ...
    try{
      ...
      else if(a==='placement-action-start'){await api(`/intelligence/institution/actions/${encodeURIComponent(id)}`,{method:'PATCH',body:JSON.stringify({status:'in_progress'})});toast('Action assigned','You now own this placement follow-up.');navigate('attention');}
      else if(a==='placement-action-resolve'){const note=window.prompt('Resolution note (optional)')||'';await api(`/intelligence/institution/actions/${encodeURIComponent(id)}`,{method:'PATCH',body:JSON.stringify({status:'resolved',resolution_note:note})});toast('Action resolved','The resolution is now part of the placement action history.');navigate('attention');}
  ...
  async function renderStudentEnterprise(view){
    setContextAction();
    if(view==='readiness'){
      setPage('Readiness evidence','Career intelligence'); const r=await api('/intelligence/student/readiness');
      $('#app-content').innerHTML=`${pageHead('Evidence-Based Readiness','A transparent view of the evidence behind your preparation. This does not predict or guarantee employment.')}<div class="readiness-hero"><div class="readiness-score ${scoreTone(r.score)}"><strong>${r.score}</strong><span>/100</span><small>${esc(r.readiness_band||'READINESS')}</small></div><div><h2>${esc(r.readiness_band||'Readiness evidence')}</h2><p>${esc(r.disclaimer)}</p><p class="muted-copy">${r.evidence_summary?.high_confidence_sources||0} of ${r.evidence_summary?.total_sources||0} evidence sources currently have high confidence.</p></div></div><div class="readiness-components">${(r.evidence||[]).map(c=>`<article><div><strong>${esc(c.label)}</strong><span>${c.score}/100</span></div><div class="professional-meter"><i style="width:${c.score}%"></i></div><small>${esc(c.confidence)} confidence · ${esc(c.source)}</small><p>${esc(c.evidence)}</p></article>`).join('')}</div>${featurePanel('Next best action','The highest-value improvement PlaceAI can justify from the evidence it currently has.',`<div class="ai-box"><div class="ai-box-head"><span>EVIDENCE → ACTION</span></div><p>${esc(r.next_best_action?.title||'Review your placement evidence.')}</p><button class="button button-primary" data-view="${esc(r.next_best_action?.view||'dashboard')}">Open next action</button></div>`)}`;
  ...
  async function renderInstitutionEnterprise(view){
    setContextAction();
    if(view==='attention'){
      setPage('Drive Rescue','Command centre'); await api('/intelligence/institution/actions/sync',{method:'POST'}); const a=await api('/intelligence/institution/actions'); const m=a.summary||{}; $('#app-content').innerHTML=`${pageHead('Drive Rescue','Detect placement exceptions, assign human action, resolve them and retain outcome evidence.')}<div class="metric-grid">${enterpriseMetric('Open actions',m.open_actions||0,'Current placement exceptions',m.open_actions?'medium':'good')}${enterpriseMetric('High priority',m.high_priority||0,'Needs prompt human review',m.high_priority?'high':'good')}${enterpriseMetric('Due within 24h',m.due_within_24h||0,'Deadline-sensitive actions',m.due_within_24h?'high':'good')}${enterpriseMetric('In progress',m.in_progress||0,'Actions with an owner')}${enterpriseMetric('Resolved · 30d',m.resolved_last_30_days||0,'Closed-loop outcomes','good')}${enterpriseMetric('Rescue drives',m.rescue_drives||0,'Drives with active exceptions')}</div>${featurePanel('Drive rescue queues','Drives with unresolved actions.',a.rescue_drives?.length?`<div class="card-grid">${a.rescue_drives.map(d=>`<article class="operation-card"><div class="operation-card-top"><span class="status-pill">${d.open_actions} open</span><small>${d.deadline?'Deadline '+fmtDateTime(d.deadline):'No deadline'}</small></div><h3>${esc(d.drive_title)}</h3><p>${esc(d.company_name||'Company')} · ${esc(d.job_title||'Role')}</p><button class="button button-secondary button-full" data-view="drives">Open drive</button></article>`).join('')}</div>`:emptyState('DR','No active rescue queues','No current drive exceptions require action.'))}${a.actions?.length?enterpriseTable(['Priority','Student / drive','Action','Due','Status',''],a.actions.map(x=>`<tr><td>${statusBadge(x.priority)}</td><td><span class="table-primary">${esc(x.student_name||'Student')}</span><span class="table-secondary">${esc(x.drive_title||x.student_branch||'Institution action')}</span></td><td><span class="table-primary">${esc(x.title)}</span><span class="table-secondary">${esc(x.description||'')}</span></td><td>${x.due_at?fmtDateTime(x.due_at):'—'}</td><td>${statusBadge(x.status)}</td><td><div class="row-actions compact">${x.status==='open'?`<button class="row-button primary" data-action="placement-action-start" data-id="${x.id}">Take ownership</button>`:''}<button class="row-button" data-action="placement-action-resolve" data-id="${x.id}">Resolve</button></div></td></tr>`).join('')):emptyState('OK','No unresolved placement actions','Drive Rescue will materialize objective exceptions as placement activity begins.')}<p class="trust-disclaimer">${esc(a.disclaimer||'')}</p>`;
